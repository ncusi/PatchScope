...
class BugDataset:
    ...
    def __init__(self, bug_ids: List[str], dataset_path: Optional[PathLike] = None,
    ...
    @classmethod
    def from_directory(cls, dataset_dir: PathLike, fan_out: bool = False) -> 'BugDataset':
        ...
        try:
            return BugDataset([str(d.name) for d in dataset_path.iterdir()
                               if d.is_dir()],
                              dataset_path=dataset_path, fan_out=fan_out)
    ...
    def get_bug(self, bug_id: str) -> Bug:
        ...
        if self._dataset_path is not None:
            return Bug.from_dataset(self._dataset_path, bug_id, fan_out=self._fan_out)
